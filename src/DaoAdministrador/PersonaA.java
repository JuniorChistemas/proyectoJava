 ...
 * @author Jesús
 */
public class PersonaA implements IPersona<PersonaEntity,PersonaDTO>{

    private final IConector conexion;
    private PreparedStatement ps;
    private CallableStatement cal;
    private ResultSet resp;
    public  PersonaA(BaseDatos obj) {
    ...
    public boolean eliminar(String obj) {
          try {
            ps = conexion.getConection().prepareStatement("{call DeletePersona(?)}");
            ps.setString(1, obj);
            int r = ps.executeUpdate();
            while (r == 1) {
                return true;
    ...
    @Override
    public PersonaEntity buscar(String IdCodigo) {
        try {
